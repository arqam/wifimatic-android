...
import org.cprados.wificellmanager.billing.BillingService.RequestPurchase;
import org.cprados.wificellmanager.billing.BillingService.RestoreTransactions;
import org.cprados.wificellmanager.billing.Consts.PurchaseState;
import org.cprados.wificellmanager.billing.Consts.ResponseCode;
    ...
    public static void purchaseResponse(
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                String sku_managed = res.getString(R.string.market_billing_donation_sku2);
                int quantity = 0;
                
                // Managed donation purchased, refunded or cancelled
                if (productId.equals(sku_managed)) {
                    if (purchaseState != PurchaseState.CANCELED) {
                        quantity = (purchaseState == PurchaseState.PURCHASED) ? 1 : 0;
                        DataManager.setHasDonated(context, (purchaseState == PurchaseState.PURCHASED));
                    }
                }
                
                // Non managed donation purchased, refunded or cancelled
                else {                
                    quantity = DataManager.getDonationsCounter(context);
                    if (purchaseState != PurchaseState.CANCELED) {
                        quantity = DataManager.setDonationsCounter(context, (purchaseState == PurchaseState.PURCHASED));
                    }
                }
